...
from awscli.utils import emit_top_level_args_parsed_event, config_secticon
...
SERVICETYPE=["s3","s3api", "configure"]
...
class CLIDriver(object):
    ...
    def __init__(self, session=None):
        ...
        self.session.get_component('data_loader')._search_paths.extend(
    ...
    def main(self, args=None):
        ...
        commands = OrderedDict()
        for cmd in SERVICETYPE:
            if cmd in command_table:
                commands[cmd] = command_table[cmd]
        parser = self._create_parser(commands)
        self._add_aliases(command_table, parser)
        if args[0] != "configure":
            config_file = self.session.get_config_variable('config_file')
            config_args =  botocore.configloader.load_config(config_file)
            args =  config_secticon(config_args, args)
