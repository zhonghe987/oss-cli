...
def config_secticon(config_args, args):
    """change default config section """
    change_item = {}
    profile = 'default'
    for item in args:
        if '=' in item:
            sp_list = item.split('=')
            change_item[sp_list[0][2:]]=sp_list[1]
            
    if 'profile' in change_item:
        profile = change_item.pop('profile')
    configdic = config_args["profiles"]
    cur_config = configdic[profile]
    for key in change_item.keys():
        cur_config.pop(key)

    for new_key in cur_config.keys():
        avail_key = '--'+new_key
        new_section = avail_key+ '=' + cur_config[new_key]
        args.append(new_section)
    return args
